...
const Dashboard = () => {
    ...
    return (
        ...
        <label className="relative block ">
            ...
            <input  className={`placeholder:text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-4 pl-9 pr-3 shadow-sm focus:outline-none focus:border-main focus:ring-1 sm:text-sm disabled:opacity-10`} placeholder="Search for company, provider, user, etc." type="text" name="search" onChange={e => setSearchValue(e.target.value)} disabled={!isAuthenticated ? true : false}/>
        </label>
